...
const mouseOverHandler = (e: Event) => {
  ...
  useResizeObserver(store.activeElement, () => store.activeElement !== undefined && moveToolbar(store.activeElement))
  window?.addEventListener('resize', () => store.activeElement !== undefined && moveToolbar(store.activeElement))
}
